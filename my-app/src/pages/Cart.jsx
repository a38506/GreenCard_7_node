...
const ADDRESS_API = import.meta.env.VITE_PAYMENT_URL || '';
...
const Cart = () => {
    ...
    const getUserAddress = async () => {
        try {
            const { data } = await axios.get(`${ADDRESS_API}/api/address/get`);
            if (data.success) {
    ...
    const placeOrder = async () => {
        try {
            ...
            if (paymentOption === 'COD') {
                const { data } = await axios.post(`${ADDRESS_API}/api/payment/cod`, {
                    userId: user._id,
                ...
                const { data } = await axios.post(`${ADDRESS_API}/api/payment/stripe`, {
                    userId: user._id,
                    items: cartArray.map(item => ({ product: item._id, quantity: item.quantity })),
                    address: selectedAddress._id,
                    amount: getCartAmount() + getCartAmount() * 2 / 100,
