...
const AddProduct = () => {
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            const { data } = await axios.post(`${PRODUCT_API}/api/product/add`, formData)
    ...
    return (
        <div className=''>
            <div className=" no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between ">
                <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5">

                    <div className="flex flex-col gap-1 max-w-md min-w-3xl">
                    ...
                    <div className='flex flex-col gap-1 w-full min-w-3xl'>
                        <p className="text-base font-medium">Product Image</p>
                        <div className="flex flex-wrap items-center gap-12 mt-2 ">
                            {Array(4).fill('').map((_, index) => (
                                <label key={index} htmlFor={`image${index}`}>
                                    ...
                                    <img className="min-w-39 cursor-pointer" src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} alt="uploadArea" width={100} height={100} />
                                </label>
                            ))}
                        </div>
                    </div>
